...
public:
  ...
  PatrolNode()
  : Node("patrol_node"), current_waypoint_index_(0), is_active_(false), loop_mode_(true)
  {
    ...
    waypoints_sub_ = this->create_subscription<geometry_msgs::msg::PoseArray>(
      "/patrol/set_waypoints", 10,
      std::bind(&PatrolNode::waypoints_callback, this, std::placeholders::_1));
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Patrol Node Initialized. Waiting for command on /patrol/cmd...");

    // 异步等待 Action 服务器启动
    if (!action_client_->wait_for_action_server(std::chrono::seconds(10))) {
      RCLCPP_ERROR(
        this->get_logger(), "Action server 'navigate_to_pose' not available after waiting");
      return;
    }
...
private:
  void cmd_callback(const std_msgs::msg::String::SharedPtr msg)
  {
    ...
    if (cmd == "start" || cmd == "start_once" || cmd == "resume") {
      ...
      is_active_ = true;
      RCLCPP_INFO(
        this->get_logger(), "Starting/Resuming patrol (loop=%s)...", loop_mode_ ? "true" : "false");
      this->send_next_goal();
    } else if (cmd == "pause") {
      if (!is_active_) {
  ...
  void send_next_goal()
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), ">>> Navigating to Waypoint %zu/%zu: X=%.2f, Y=%.2f",
  ...
  void get_result_callback(const GoalHandleNav::WrappedResult & result)
  {
    switch (result.code) {
      case rclcpp_action::ResultCode::SUCCEEDED:
        RCLCPP_INFO(
          this->get_logger(), "<<< Successfully reached Waypoint %zu!",
